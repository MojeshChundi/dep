...
function logIns(e) {
  ...
  axios
    .post("http://54.145.29.149:3000/user/login", userDetails)
    .then((res) => {
      console.log(res.data.user[0].ispremium);
      localStorage.setItem("token", res.data.token);
      localStorage.setItem("ispremium", res.data.user[0].ispremium);
      window.alert("login successfully!!");
      window.location.href = "http://54.145.29.149:3000/public/expense.html";
    })
    .catch((err) => {
...
function forgotMail() {
  window.location.href = "http://54.145.29.149:80/public/forgotMail.html";
}
